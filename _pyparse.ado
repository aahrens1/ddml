program _pyparse, rclass
	syntax [anything] , type(string) method(string)

	if "`type'"=="reg" {
		if "`method'"=="lasso" {
			parse_LassoIC `anything'
		}
		if "`method'"=="svm" {
			parse_SVR `anything'
		}
		else if "`method'"=="gradboost" {
			parse_gradboostReg `anything'
		}
		else if "`method'"=="rf" {
			parse_rfReg `anything'
		}
		...
		if "`method'"=="svm" {
			...
			parse_gradboostClass `anything'
		}
		else if "`method'"=="rf" {
			parse_rfClass `anything'
		}
		local optstr `r(optstr)'
	}
...
program define parse_LinearSVC, rclass
	...
	** multi class 
	if "`multi_class'"=="ovr"|"`multi_class'"=="crammer_singer" {
		local optstr `optstr' 'multi_class':'`multi_class'',
	}
	else {
		local optstr `optstr' 'multi_class':'ovr',
	}
	** class_weight
	if "`class_weight'"=="balanced" {
		local optstr `optstr' 'class_weight':'balanced',
	}
	else {
