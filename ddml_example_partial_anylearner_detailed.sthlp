...
{phang2}. {stata "ddml init partial, kfolds(2) reps(5) mname(m1) prefix"}{p_end}
...
{phang2}. {stata "ddml E[Y|X], mname(m1): reg $Y $X"}{p_end}
...
{phang2}. {stata "ddml E[Y|X], mname(m1): pystacked $Y $X, type(reg) method(rf)"}{p_end}

{pstd}We do the same for the conditional expectation E[D|X].{p_end}

{phang2}. {stata "ddml E[D|X], mname(m1): reg $D $X"}{p_end}
{phang2}. {stata "ddml E[D|X], mname(m1): pystacked $D $X, type(reg) method(rf)"}{p_end}
...
{phang2}. {stata "gen double Yresid = $Y - m1_Y1_reg_4"}{p_end}
{phang2}. {stata "gen double Dresid = $D - m1_D1_reg_4"}{p_end}
...
{phang2}. {stata "twoway (scatter $Y m1_Y2_pystacked_4) "}{p_end}
